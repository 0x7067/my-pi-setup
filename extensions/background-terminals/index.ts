 ...
 *
 * While ≥1 process runs, the fixed-height footer shows
 * "N background terminal(s) · /ps". `/ps` opens a two-stage
...
export default function (pi: ExtensionAPI) {
  ...
  /** Footer status shown only while ≥1 terminal is running.
  ...
  const updateStatus = (manager: TerminalManagerShape) => {
    ...
    try {
      ...
      ui.setStatus(
        ...
        ui.theme.fg(
          "warning",
          `${running} background terminal${running === 1 ? "" : "s"} · /ps`,
        ),
      );
    } catch {
