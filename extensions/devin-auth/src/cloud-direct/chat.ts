...
import { getOrAllocateSessionIds } from "./session-ids.js";

export { allocateCascadeId, clearSessionIds } from "./session-ids.js";
 ...
 *   #5 safe_for_code_telemetry: bool            (0 = do not log)
 ...
 */
function collapseSystemIntoUser(
...
function normalizeContent(
  ...
  for (const p of parts) {
    ...
    if (p.type === "text" && typeof p.text === "string") {
      ...
      const mimeType =
        typeof p.mimeType === "string" ? p.mimeType : "image/png";
      ...
      const url: string =
        typeof imgRef === "string" ? imgRef : (imgRef.url ?? "");
  ...
  for (const f of iterFields(buf)) {
    ...
    if (
      ...
      if (entryMetric === "input_tokens") promptTokens = n;
        ...
        entryMetric === "cached_input_tokens" ||
        entryMetric === "cache_read_input_tokens"
      ) {
        ...
        entryMetric === "reasoning_tokens" ||
        entryMetric === "output_reasoning_tokens"
      ) {
...
export interface CloudChatRequest {
  ...
  cascadeId?: string;
  /** Stable Pi conversation ID used to isolate server-side cache identity. */
...
export async function* streamChatEvents(
  ...
  const catalog = await getCachedCatalog(req.apiKey, host, req.signal).catch(
    () => null,
  );
  if (catalog) {
  ...
  // Derive stable, distinct IDs per Pi conversation. Falling back to the
  // account-scoped process cache preserves compatibility for non-Pi callers.
  const sessionIds = getOrAllocateSessionIds(
    req.apiKey,
    host,
    req.conversationId,
    req.cascadeId,
  ...
  const ttfbTimer = setTimeout(
    () =>
      ttfbController.abort(
        new Error(
          `cloud-direct: time-to-first-byte timeout (${CLOUD_STREAM_TTFB_MS}ms)`,
        ),
      ),
  ...
  const initialSignal: AbortSignal = req.signal
    ? anySignal([req.signal, ttfbSignal])
    : ttfbSignal;
  ...
  const reader =
    resp.body.getReader() as ReadableStreamDefaultReader<Uint8Array>;
  let trailerError: {
    code?: string;
    message: string;
    traceId?: string;
      ...
      idleTimer = setTimeout(
        () =>
          idleController.abort(
            new Error(
              `cloud-direct: idle timeout (${CLOUD_STREAM_IDLE_MS}ms with no bytes)`,
            ),
          ),
          ...
          () => {
            try {
              void resp.body?.cancel(
                idleController.signal.reason ?? new Error("idle abort"),
              );
            } catch {
              /* */
            }
      ...
      while (queuedBytes >= 5) {
        ...
        if (flags & 0x01) {
          try {
            ...
            throw new CloudChatError(
              `Connect frame gunzip failed: ${(gzipErr as Error).message}`,
            );
          }
        }
