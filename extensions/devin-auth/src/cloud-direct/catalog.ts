...
async function fetchCatalog(
  ...
  const timer = setTimeout(
    () =>
      ac.abort(
        new Error(`catalog: fetch timeout (${CATALOG_FETCH_TIMEOUT_MS}ms)`),
      ),
  ...
  try {
    resp = await fetch(
      `${host}/exa.api_server_pb.ApiServerService/GetCascadeModelConfigs`,
      {
        method: "POST",
        headers: {
  ...
  if (!resp.ok) {
    ...
    throw new Error(
      `GetCascadeModelConfigs HTTP ${resp.status}: ${text.slice(0, 200)}`,
    );
  }
