  ...
  it("keys the returned snapshot tag on what the Filesystem actually persisted, not the pre-write content", async () => {
    ...
    expect(
      section.warnings.some(
        (w) => w.includes(PATH) && /reformatted it on save/.test(w),
      ),
    ).toBe(true);
    ...
    await patcher.apply(
      Patch.parse(`[${PATH}#${section.fileHash}]\nPUT 1-1:\n+function g() {`),
    );
...
describe("Patcher mandatory snapshot tag policy", () => {
  it("rejects a hashless head/tail insert — the tag is required on every section", async () => {
    ...
    await expect(
      patcher.apply(Patch.parse(`[${PATH}]\nPUT >$:\n+c`)),
    ).rejects.toThrow(/Missing hashline snapshot tag.*use the write tool/s);
  ...
  it("rejects a tagged edit whose target file does not exist (create with write instead)", async () => {
    ...
    await expect(
      patcher.apply(Patch.parse(`[ghost.ts#1A2B]\nPUT >$:\n+c`)),
    ).rejects.toThrow(/File not found.*use the write tool/is);
  });
