...
> = {
  replace: {
    single: (line, reg) =>
      reg ? `PUT ${line}${regSuffix(reg)}` : `PUT ${line}:`,
    range: (start, end, reg) =>
      reg
  ...
  cut: {
    ...
    range: (start, end, reg) =>
      `CUT ${start}${HL_RANGE_SEP}${end}${regSuffix(reg)}`,
  },
};
 ...
 */
function unresolvedLoweredWarning(
  blockForm: string,
  line: number,
  plainForm: string,
): string {
 ...
 */
export function ambiguousBoundaryPlacementMessage(
  startLine: number,
  endLine: number,
): string {
  return (
 ...
 * compiler pass.
 */
export function editBrokeParseWarning(
  firstChangedLine: number | undefined,
): string {
  const at =
    firstChangedLine === undefined ? "" : ` near line ${firstChangedLine}`;
  return (
...
export function emptyRegisterPasteWarning(
  name: string,
 ...
 */
export function emptyRegisterSpanPasteMessage(
  name: string,
  known: readonly string[],
): string {
  const base =
...
export function ambiguousAnonymousPasteMessage(
