...
function insertEditAt(
  ...
  if (edit?.kind !== "insert") {
    throw new Error(
      "internal error: after-insert group contains a non-insert edit",
    );
  }
  return edit;
}
  ...
  for (let start = 0; start < edits.length;) {
    ...
    for (let offset = 0; offset < group.payload.length; offset++) {
      ...
      if (
        ...
        source.trimStart() !== payload.trimStart()
      )
        continue;
      ...
      const candidate = sourceIndent.slice(
        0,
        sourceIndent.length - payloadIndent.length,
      );
    ...
    if (
      ...
      !shift ||
      matches < 2 ||
      matches * 2 <= group.payload.length
    )
      continue;
...
function normalizeTextualBoundaryEchoes(
  ...
  while (i < edits.length) {
    ...
    if (dropLeading > 0 || dropTrailing > 0) {
      out.push(
        ...inserts.slice(dropLeading, inserts.length - dropTrailing),
        ...deletes,
      );
      warnings.push(
        textualBoundaryEchoWarning(group.startLine, dropLeading, dropTrailing),
      );
    } else {
      for (const idx of group.insertIndices)
        out.push(cloneAppliedEdit(edits[idx], idx));
...
function nearestContentLine(
  ...
  for (
    let index = start;
    index >= 0 && index < fileLines.length;
    index += step
  ) {
...
function isSourceLineDeleted(
  edits: readonly AppliedEdit[],
  line: number,
): boolean {
  return edits.some(
    (edit) => edit.kind === "delete" && edit.anchor.line === line,
  );
}
...
function isSyntaxEssentialRow(
  ...
  const without = [
    ...fileLines.slice(0, line - 1),
    ...fileLines.slice(line),
  ].join("\n");
...
function edgeEvidence(
  ...
  const leadingStructure =
    ...
    enclosingBoundaries(fileLines, path, innerStart, trailingLine).includes(
      group.startLine,
    );
...
function buildKeepPlans(
  ...
  const underfilledEffectiveEdge =
    trailingLine < group.endLine &&
    payload.length < group.endLine - group.startLine + 1;
  const keepsLeading =
    evidence.first &&
...
function buildGroupVariants(
  ...
  for (const leadingDrop of leadingDrops) {
    for (const trailingDrop of trailingDrops) {
      ...
      const payload = group.payload.slice(
        leadingDrop,
        group.payload.length - trailingDrop,
      );
...
function applyGroupVariant(
  ...
  let retainedInserts = inserts.slice(
    dropLeading,
    inserts.length - dropTrailing,
  );
...
function repairBoundaryVariants(
  ...
  while (i < edits.length) {
    const group = findReplacementGroup(edits, i);
    if (group) {
      const built = buildGroupVariants(
        group,
        edits,
        fileLines,
        path,
      ...
      if (built.ambiguous && ambiguousGroup === undefined)
        ambiguousGroup = group;
      if (built.variants.length > 0)
        groups.push({ group, variants: built.variants });
      i = group.deleteIndices[group.deleteIndices.length - 1] + 1;
    } else {
      i++;
    }
  ...
  let combos: BoundaryCombo[] = [
    { variants: [], touched: 0, kept: 0, dropped: 0 },
  ];
...
function repairAfterInsertLandings(
  ...
  for (const group of groups.values()) {
    const target = bodyTargetIndent(
      group.members.map((idx) => insertEditAt(edits, idx).text),
    );
...
function materializeEdits(
  ...
  const { edits: landed, warnings } = repairAfterInsertLandings(
    edits,
    originalLines,
  );
...
export function applyEdits(
  ...
  const concrete = resolveClipboardEdits(
    edits,
    fileLines,
    options.clipboard ?? {},
    {
      ...(options.onEmptyPaste === undefined
        ? {}
        : { onEmptyPaste: options.onEmptyPaste }),
  ...
  const indentationWarnings = repairReplacementIndentation(
    targetEdits,
    fileLines,
  );
