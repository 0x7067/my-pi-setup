...
const { diffLineRuns } = loadNatives();
...
function remapEditsToCurrent(
	...
	if (!validateRemappedAnchorContext(previousText, currentText, lineMap, edits))
		return null;
	...
	for (const edit of edits) {
		...
		if (edit.kind === "cut") {
			...
			let end = start;
			for (
				let line = edit.range.start.line + 1;
		...
		if (edit.kind === "paste") {
			...
			if (edit.at.kind === "span") {
				...
				let end = start;
				for (
					let line = edit.at.range.start.line + 1;
					line <= edit.at.range.end.line;
					line++
				) {
					const mapped = mapLine(line);
			...
			if (cursor.kind !== "before_anchor" && cursor.kind !== "after_anchor") {
				remapped.push(
					blockStart === edit.blockStart ? edit : { ...edit, blockStart },
				);
				continue;
			}
