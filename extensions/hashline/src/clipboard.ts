...
function describeCutEdit(edit: CutEdit): string {
  ...
  const span =
    start.line === end.line
      ? `${start.line}`
      : `${start.line}${HL_RANGE_SEP}${end.line}`;
...
export function hasClipboardEdit(edits: readonly Edit[]): boolean {
  return edits.some(
    (edit) =>
      ...
      (edit.kind === "block" &&
        (edit.mode === "cut" ||
          edit.mode === "paste_after" ||
          edit.register !== undefined)),
  );
}
...
function readRegister(
  ...
  if (register !== undefined) {
    ...
    if (target === "span") {
      throw new Error(
        `line ${lineNum}: ${emptyRegisterSpanPasteMessage(register, known)}`,
      );
    }
    onWarning?.(
      `line ${lineNum}: ${emptyRegisterPasteWarning(register, known)}`,
    );
    return [];
  }
  ...
  if (pending.length > 1) {
    if (onEmptyPaste === "drop") return null;
    throw new Error(
      `line ${lineNum}: ${ambiguousAnonymousPasteMessage(pending)}`,
    );
  }
 ...
 */
export function validateClipboardSequence(
  edits: readonly Edit[],
  clipboard: Clipboard,
): void {
