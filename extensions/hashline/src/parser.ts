 ...
 */
function bodylessTargetMessage(
  target: BlockTarget,
  hadColon: boolean,
): string | null {
  if (target.kind === "cut" || target.kind === "cut_block")
...
function parseTopLevelSnapshotRow(
  text: string,
...
const TOP_LEVEL_BARE_RANGE_HEADER_RE =
  /^\s*([1-9]\d*)(?:\s|[-.=…])+([1-9]\d*)\s*:\s*$/;
...
export class Executor {
  ...
  endStreaming(): { edits: Edit[]; fileOp?: FileOp; warnings: string[] } {
    ...
    if (
      pending &&
      (pending.payloads.length > 0 || this.#isCompleteBodylessOp(pending))
    )
      this.#flushPending();
  ...
  #normalizeOverlappingRanges(): void {
    ...
    for (const edit of this.#edits) {
      ...
      if (edit.kind === "paste" && edit.at.kind === "span") {
        ...
        for (
          let line = edit.at.range.start.line;
          line <= edit.at.range.end.line;
          line++
        ) {
      ...
      if (edit.kind === "delete")
        hunkFor(edit.lineNum).sourceLines.add(edit.anchor.line);
    }
    ...
    for (const hunk of hunks.values()) {
      ...
      const previous =
        overlaps.size === 1 ? overlaps.values().next().value : undefined;
      const exact =
        previous !== undefined &&
  ...
  #handleLiteralPayload(text: string, lineNum: number): void {
    ...
    const noBodyOnLiteral = bodylessTargetMessage(
      pending.target,
      pending.hadColon,
    );
    if (noBodyOnLiteral !== null)
      throw new Error(`line ${lineNum}: ${noBodyOnLiteral}`);
    ...
    if (isHunkHeaderText(text))
      this.#warnings.push(literalOpRowWarning(lineNum, text));
  ...
  #handleRaw(text: string, lineNum: number): void {
    ...
    if (this.#pending) {
      ...
      const noBodyOnRaw = bodylessTargetMessage(
        this.#pending.target,
        this.#pending.hadColon,
      );
      if (noBodyOnRaw !== null)
        throw new Error(`line ${lineNum}: ${noBodyOnRaw}`);
    ...
    if (snapshotRow !== null) {
      ...
      if (this.#recoveredSnapshotLines.has(snapshotRow.line)) {
        throw new Error(
          `line ${lineNum}: ${repeatedSnapshotRowMessage(snapshotRow.line)}`,
        );
      }
      this.#recoveredSnapshotLines.add(snapshotRow.line);
      const range = {
        start: { line: snapshotRow.line },
        end: { line: snapshotRow.line },
      };
  ...
  #handleBlank(text: string, lineNum: number): void {
    const pending = this.#pending;
    if (!pending) return;
    if (bodylessTargetMessage(pending.target, pending.hadColon) !== null)
      return;
  ...
  #flushPending(): void {
    ...
    if (target.kind === "replace") {
      ...
      if (payloads.length === 0) {
        if (!hadColon)
          throw new Error(`line ${lineNum}: ${COLONLESS_SPAN_PUT}`);
