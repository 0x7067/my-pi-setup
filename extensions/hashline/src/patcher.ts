...
function hasAnchorScopedEdit(edits: readonly Edit[]): boolean {
  return edits.some((edit) => {
    if (edit.kind === "delete" || edit.kind === "block" || edit.kind === "cut")
      return true;
    if (edit.kind === "paste") {
...
export class Patcher {
  ...
  async apply(patch: Patch): Promise<PatcherApplyResult> {
    ...
    for (let index = 0; index < prepared.length; index++) {
      ...
      if (this.clipboard !== undefined)
        commitClipboard(sectionStates[index], this.clipboard);
    }
  ...
  async #parseWithRangeDiagnostics(section: PatchSection) {
    try {
      ...
      if (!(error instanceof InvalidAbsoluteRangeError) || !this.blockResolver)
        throw error;
  ...
  async commit(prepared: PreparedSection): Promise<PatchSectionResult> {
    ...
    const allWarnings = driftedOnWrite
      ? [...warnings, writeDriftWarning(section.path)]
      : warnings;
  ...
  #applyWithRecovery(args: {
    ...
    const { section, canonicalPath, exists, normalized, edits, clipboard } =
      args;
