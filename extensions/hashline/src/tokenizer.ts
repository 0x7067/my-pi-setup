...
function scanRangeSeparator(
  ...
  if (
    !consumedSeparator ||
    cursor >= end ||
    !isNonZeroDigitCode(line.charCodeAt(cursor))
  )
    return null;
  return cursor;
}
 ...
 */
function scanDanglingSeparator(
  line: string,
  index: number,
  end: number,
): number | null {
...
type RegisterableTarget = Exclude<
  BlockTarget,
  { kind: "rem" } | { kind: "move" }
>;
...
function scanPutTarget(
  ...
  return finishTargetScan(line, next, end, {
    kind: "replace",
    range: range.range,
  });
}
