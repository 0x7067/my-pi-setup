...
/** Repetition penalty for OCR transcription (benchmarked on GLM-OCR). */
...
/** Model ids for both workers. GLM-OCR is the sole transcription model. */
...
export function missingModels(
	models: Record<WorkerName, string> = WORKER_MODELS,
) {
	return Object.values(models).filter((modelId) => !isModelInstalled(modelId));
}
