...
test("a spill-file failure does not discard a successful read", async () => {
  const calls: string[] = [];
  const deps = makeDeps(
    {
      runPrivate: async () => [
        {
          page: 1,
          text: Array.from(
            { length: OUTPUT_MAX_LINES + 50 },
            (_, index) => `line ${index} ${"x".repeat(30)}`,
          ).join("\n"),
        },
      ],
      saveFullResult: async () => {
  ...
  const outcome = await executeParse({ path: "scan.png" }, deps);
  assert.equal(outcome.truncated, true);
  assert.equal(outcome.fullResultPath, undefined);
  assert.ok(outcome.text.includes("could not be saved"));
  assert.ok(Buffer.byteLength(outcome.text, "utf8") <= OUTPUT_MAX_BYTES);
  assert.ok(outcome.text.split("\n").length <= OUTPUT_MAX_LINES);
});
