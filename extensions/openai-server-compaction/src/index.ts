...
function getMatchingRemoteState(
  ...
  return remoteState && remoteState.modelKey === modelKey(model)
    ? remoteState
    : undefined;
}
...
function maybeNotifyRequestFeatures(params: {
  ...
  params.ui.notify(
    `OpenAI compaction active for ${key} (${params.features.join(", ")})`,
    "info",
  );
}
...
export default function openaiServerCompactionExtension(pi: ExtensionAPI) {
  ...
  const clearBeforeSessionChange = (
    _event: unknown,
  ...
  pi.on("session_before_compact", async (event, ctx) => {
    ...
    if (!cfg.enabled || !model || !supportsRemoteCompactionModel(model))
      return undefined;
  ...
  pi.on("message_end", (event, ctx) => {
    ...
    setContinuationState(sessionId, {
      ...
      updatedAt: Date.now(),
      contextLength: getBranchMessageCount(
