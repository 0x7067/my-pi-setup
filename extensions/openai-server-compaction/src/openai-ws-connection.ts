...
export class OpenAIWebSocketManager extends EventEmitter {
  ...
  private async createSocket(): Promise<any> {
    if (!this.apiKey)
      throw new Error("OpenAIWebSocketManager: apiKey is required.");
    ...
    return await new Promise<void>(async (resolve, reject) => {
      ...
      const onClose = (code: number, reason: Buffer | string) => {
        ...
        if (!settled) {
          finishReject(
            new Error(
              `OpenAIWebSocketManager: connection closed before open (code=${code}, reason=${text || "unknown"})`,
            ),
          );
          return;
        }
    ...
    if (!parsed || typeof parsed !== "object" || !("type" in parsed)) {
      if (this.listenerCount("error") > 0) {
        this.emit(
          "error",
          new Error(
            `OpenAIWebSocketManager: unexpected message shape: ${text.slice(0, 200)}`,
          ),
        );
      }
      return;
