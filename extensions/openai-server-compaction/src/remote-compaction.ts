...
export type ResponseItem =
  ...
  | {
      ...
      call_id: string;
      output: string | ToolResultOutputItem[];
    }
...
function withRemoteCompactionV2Feature(
  ...
  const features = [
    ...new Set([...configuredFeatures, REMOTE_COMPACTION_V2_FEATURE]),
  ];
  return {
...
export function buildRemoteCompactionHeaders(params: {
  ...
  throw new Error(
    "Remote compaction v2 headers are not supported for this model.",
  );
}
...
function contentToResponseContentItems(
  ...
  for (const part of content as ContentPartLike[]) {
    ...
    if (
      ...
      items.push({
        type: "input_image",
        image_url: (part.source as { url: string }).url,
      });
    }
  }
  return items;
  ...
  try {
    ...
    const summary = Array.isArray(parsed.summary)
      ? parsed.summary
          ...
          .filter((item): item is { type: "summary_text"; text: string } =>
            Boolean(item),
          )
      : [];
    const content = Array.isArray(parsed.content)
      ? parsed.content
          ...
          .filter(
            (item): item is { type: "reasoning_text" | "text"; text: string } =>
              Boolean(item),
          )
      : undefined;
...
function buildPortableSummaryPrompt(
  conversation: string,
...
export function messageToResponseItems(message: AgentMessage): ResponseItem[] {
  ...
  if (message.role === "assistant") {
    ...
    for (const block of message.content) {
      ...
      flushText();
      const callId =
        typeof block.id === "string" ? block.id.split("|", 1)[0] : block.id;
      items.push({
        type: "function_call",
        name: block.name,
...
function stripImagesWhenUnsupported(
  ...
  return items.map((item) => {
    ...
    if (next.type === "message" && Array.isArray(next.content)) {
      ...
      next.output = stripUnsupportedFunctionOutputImages(
        responseItemOutput(next),
      );
    } else if (
...
export function buildRemoteCompactionV2History(
  ...
  if (compactionItem.type !== "compaction") {
    throw new Error(
      "OpenAI remote compaction v2 did not return a compaction item.",
    );
  }
...
function parseUsageCostSnapshot(
  value: unknown,
): RemoteCompactionUsageSnapshot["cost"] | undefined {
  if (!isRecord(value)) return undefined;
  const input =
  ...
  const output =
    typeof value.output === "number" && Number.isFinite(value.output)
      ...
      : 0;
  const cacheRead =
    typeof value.cacheRead === "number" && Number.isFinite(value.cacheRead)
      ? value.cacheRead
      : 0;
  const cacheWrite =
    typeof value.cacheWrite === "number" && Number.isFinite(value.cacheWrite)
      ? value.cacheWrite
      : 0;
  const total =
...
function parseRemoteCompactionUsageSnapshot(
  value: unknown,
): RemoteCompactionUsageSnapshot | undefined {
  if (!isRecord(value)) return undefined;
  const input =
...
export function extractRemoteCompactionDetails(
  ...
  const isLegacy =
    remote.provider === "openai-responses-compact" && remote.version === 1;
  const isV2 =
    remote.provider === "openai-responses-compaction" && remote.version === 2;
...
export function reconstructRemoteCompactionStateFromBranch(params: {
  ...
  for (const entry of params.branchEntries.slice(latestCompactionIndex + 1)) {
    ...
    if (entry.message.role === "assistant") {
      if (
        assistantMessageMatchesModelKey(entry.message, latestDetails.modelKey)
      ) {
