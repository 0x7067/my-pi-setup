...
function normalizeAssistantPhase(
  value: unknown,
): OpenAIResponsesAssistantPhase | undefined {
...
function supportsImageInput(modelOverride?: ReplayModelInfo): boolean {
  return (
    !Array.isArray(modelOverride?.input) ||
    modelOverride.input.includes("image")
  );
}
...
function parseReasoningItem(
  value: unknown,
): Extract<InputItem, { type: "reasoning" }> | null {
...
function convertMessagesToInputItems(
  ...
  for (const msg of messages) {
      ...
      if (Array.isArray(m.content)) {
        ...
        for (const block of m.content as Array<{
          ...
          if (block.type === "text" && typeof block.text === "string") {
            const parsedSignature = parseAssistantTextSignature(
              block.textSignature,
            );
          ...
          if (block.type === "thinking") {
            pushAssistantText();
            const reasoningItem = parseThinkingSignature(
    ...
    const toolCallId =
      toNonEmptyString(m.toolCallId) ?? toNonEmptyString(m.toolUseId);
    if (!toolCallId) continue;
    const [callId] = toolCallId.split("|", 2);
    const parts = Array.isArray(m.content)
      ? contentToOpenAIParts(m.content, modelOverride)
      : [];
    ...
    items.push({
      ...
      call_id: callId,
      output:
        textOutput || (imageParts.length > 0 ? "(see attached image)" : ""),
    });
...
export function selectInputItemsForContinuation(params: {
  ...
  if (
    remoteCompactionState &&
    remoteCompactionState.modelKey === currentModelKey
  ) {
...
function buildResponseCreatePayload(params: {
  ...
  return {
    ...
    ...(params.options?.maxTokens !== undefined
      ? { max_output_tokens: params.options.maxTokens }
      : {}),
    ...(params.options?.topP !== undefined
      ? { top_p: params.options.topP }
      : {}),
    ...(params.options?.toolChoice !== undefined
      ? { tool_choice: params.options.toolChoice }
      : {}),
    ...
    ...(params.options?.metadata
      ? { metadata: params.options.metadata as Record<string, string> }
      : {}),
  };
}
...
async function fallbackToHttp(
  ...
  const remoteCompactionState = sessionId
    ? getRemoteCompactionState(sessionId)
    : undefined;
  const continuationState = sessionId
    ? getContinuationState(sessionId)
    : undefined;
  ...
  const mergedOptions = {
    ...
    onPayload: async (payload: unknown, payloadModel: Model<any>) => {
      ...
      if (payload && typeof payload === "object") {
        ...
        if (
          remoteCompactionState &&
          remoteCompactionState.modelKey === modelKey(model)
        ) {
          ...
          const newMessages = context.messages.slice(
            continuationState.contextLength,
          );
          if (newMessages.length > 0) {
            payloadObj.input = convertMessagesToInputItems(
              newMessages,
              model,
            ) as unknown[];
            nextPayload = payloadObj;
...
async function fallbackToHttpResponses(
  ...
  return await fallbackToHttp(
    asResponsesModel(model),
    context,
    options,
    eventStream,
    signal,
  ...
  return (model, context, options) => {
    ...
    queueMicrotask(() => {
      const run = async () => {
        ...
        if (
          !cfg.enabled ||
          !cfg.usePreviousResponseId ||
          !isDirectOpenAIResponsesModel(model)
        ) {
          return await fallbackToHttpResponses(
            model,
            context,
            options,
            eventStream,
        ...
        const incrementalAllowed =
          session.lastRequestKey === undefined ||
          ...
          remoteCompactionState.modelKey === currentModelKey
            ...
            : incrementalAllowed
              ? (session.manager.previousResponseId ??
                (continuationState?.modelKey === currentModelKey
