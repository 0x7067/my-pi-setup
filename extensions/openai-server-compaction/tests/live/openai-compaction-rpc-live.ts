...
const extraExtensionPaths = (
  process.env.PI_OPENAI_SERVER_COMPACTION_TEST_EXTENSIONS ?? ""
)
  .split(delimiter)
  .filter(Boolean);
const primaryModel =
  process.env.PI_OPENAI_SERVER_COMPACTION_TEST_MODEL ?? "openai/gpt-5.4-nano";
const primaryModelSeparator = primaryModel.indexOf("/");
const primaryModelProvider =
  primaryModelSeparator >= 0
    ? primaryModel.slice(0, primaryModelSeparator)
    : "openai";
const primaryModelId =
  primaryModelSeparator >= 0
    ? primaryModel.slice(primaryModelSeparator + 1)
    : primaryModel;
...
function chooseAltModel(
  ...
  if (sameFamily.length > 0) {
    for (const wanted of [
      ...
      const match = sameFamily.find(
        (model) => model.id === wanted && model.id !== currentId,
      );
  ...
  for (const [key, entryValue] of Object.entries(value)) {
    redacted[key] =
      key === "encrypted_content"
        ? "<encrypted>"
        : redactEncryptedContent(entryValue);
  }
  return redacted;
}
...
class PiRpcClient {
  ...
  constructor(sessionDir: string, sessionFile?: string, cwd?: string) {
    ...
    for (const extraExtensionPath of extraExtensionPaths) {
      args.push("-e", extraExtensionPath);
    }
    if (sessionFile) {
  ...
  async send(
    ...
    return await new Promise<RpcResponse>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(
          new Error(
            `RPC command timed out: ${String(payload.type)} (${timeoutMs}ms)`,
          ),
        );
      }, timeoutMs);
      ...
      if (response.success !== true) {
        throw new Error(
          `RPC command failed: ${String(payload.type)}: ${String(response.error)}`,
        );
      }
      return response;
    });
  ...
  async getState(): Promise<JsonObject> {
    return asRecord(
      (await this.send({ type: "get_state" }, 30_000)).data,
      "get_state.data",
    );
  }
  ...
  async getMessages(): Promise<unknown[]> {
    const data = asRecord(
      (await this.send({ type: "get_messages" }, 30_000)).data,
      "get_messages.data",
    );
  ...
  async getAvailableModels(): Promise<ModelInfo[]> {
    ...
    return asArray(data.models, "get_available_models.data.models").map(
      (model) => asRecord(model, "available model") as ModelInfo,
    );
  }
  ...
  async waitIdle(timeoutMs = 240_000): Promise<JsonObject> {
    ...
    throw new Error(
      `Timed out waiting for Pi to become idle after ${timeoutMs}ms`,
    );
  }
...
async function runSameProcessTest(
  sessionDir: string,
  workspaceDir: string,
): Promise<void> {
  ...
  try {
    ...
    expect(
      altModel,
      "Could not find alternate OpenAI Responses model for /model test",
    );
    ...
    expect(
      ...
      `Summary still contains secret; test is inconclusive. Summary: ${summary}`,
    );

    const remoteCompaction = nestedRecord(
      nestedRecord(compactData.details).remoteCompaction,
    );
    expect(
    ...
    expect(
      ...
      "Missing remoteCompaction replacementHistory",
    );
    const replacementArtifact = replacementHistory.at(-1);
    expect(
      isRecord(replacementArtifact) &&
        replacementArtifact.type === "compaction",
      "Replacement history must end with a compaction v2 artifact",
    );

    const stateAfterCompact = await client.getState();
    const sessionFile = asString(
      stateAfterCompact.sessionFile,
      "get_state.data.sessionFile",
    );
    ...
    expect(
      ...
      "No compaction entry written to session file",
    );

    const lastCompaction = compactionEntries[compactionEntries.length - 1];
    const persistedRemote = nestedRecord(
      nestedRecord(lastCompaction.details).remoteCompaction,
    );
    expect(
    ...
    expect(
      ...
      `Expected secret recovery after compaction; got: ${answer}`,
    );
    ...
    const cost2 = asNumber(
      stats2.cost,
      "get_session_stats.data.cost after switch-back",
    );
    expect(
      cost2 >= cost1,
      "Expected cost to stay non-decreasing after more turns",
    );
...
async function runReducedPlaintextReplayTest(
  sessionDir: string,
  workspaceDir: string,
): Promise<void> {
  ...
  try {
    ...
    expect(
      /^[A-Z]+-[0-9]+-[A-Z]+$/.test(secret),
      `Assistant did not return a strict codename: ${secret}`,
    );
    ...
    const compactResponse = await client.send(
      {
        type: "compact",
        customInstructions:
          "Create a useful summary, but redact any exact identifiers or codewords invented earlier.",
      },
      240_000,
    );
    const compactData = asRecord(
      compactResponse.data,
      "reduced-plaintext compact.data",
    );
    void asString(
      compactData.summary,
      "reduced-plaintext compact.data.summary",
    );
    ...
    expect(
      replacementHistory.length > 0,
      "Missing replacementHistory in reduced-plaintext test",
    );
    const replacementArtifact = replacementHistory.at(-1);
    expect(
      isRecord(replacementArtifact) &&
    ...
    const visibleHistory = JSON.stringify(
      redactEncryptedContent(replacementHistory),
    );
    expect(
...
async function runForkTest(
  ...
  try {
    ...
    await client.send({
      type: "prompt",
      message: "First forkable prompt. Reply FIRST-OK.",
    });
    await client.waitIdle();
    await client.send({
      type: "prompt",
    ...
    await client.waitIdle();
    await client.send(
      { type: "compact", customInstructions: "Summarize briefly." },
      240_000,
    );
    ...
    const entryId = asString(
      asRecord(forkMessages[0], "fork message").entryId,
      "fork message entryId",
    );
    await client.send({ type: "fork", entryId }, 60_000);
    await client.send({
      type: "prompt",
      message: "Reply with exactly FORK-OK.",
    });
    await client.waitIdle();

    const answer = assistantText(await client.getMessages());
    expect(
      answer.includes("FORK-OK"),
      `Forked session did not answer correctly: ${answer}`,
    );
...
async function runResumeTest(
  ...
  try {
    ...
    sessionFile = asString(
      state.sessionFile,
      "resume get_state.data.sessionFile",
    );
...
async function runResumeAfterModelSwitchTest(
  sessionDir: string,
  workspaceDir: string,
): Promise<void> {
  ...
  try {
    ...
    expect(
      altModel,
      "Could not find alternate OpenAI Responses model for resume-after-switch test",
    );
    ...
    const compactData = asRecord(
      compactResponse.data,
      "resume-after-switch compact.data",
    );
    const summary = asString(
      compactData.summary,
      "resume-after-switch compact.data.summary",
    );
    expect(
      !summary.includes(secret),
    ...
    await client.send({
      ...
      provider: asString(
        altModel.provider,
        "resume-after-switch alt model provider",
      ),
    ...
    sessionFile = asString(
      state.sessionFile,
      "resume-after-switch get_state.data.sessionFile",
    );
    expect(
      sessionFile.length > 0,
      "Missing session file for resume-after-switch test",
    );
  } finally {
    await client.close();
  }
...
async function runNativeFallbackTest(
  sessionDir: string,
  workspaceDir: string,
): Promise<void> {
  console.log(`== native fallback test (${primaryModel}) ==`);
  const client = new PiRpcClient(sessionDir, undefined, workspaceDir);
  const secret = "NATIVE-FALLBACK-47-EMBER";
  try {
    await client.waitIdle();
    await client.send({
      type: "prompt",
      message: `Remember that the project codename is ${secret}. Reply only with MEMORIZED.`,
    });
    await client.waitIdle();
    await client.send({
      type: "prompt",
    ...
    const compactResponse = await client.send(
      {
        type: "compact",
        customInstructions: "Preserve exact project identifiers and codewords.",
      },
      240_000,
    );
    const compactData = asRecord(compactResponse.data, "native compact.data");
    void asString(compactData.summary, "native compact.data.summary");
    const details = nestedRecord(compactData.details);
    expect(
      details.remoteCompaction === undefined,
      "Non-OpenAI model unexpectedly produced OpenAI remote compaction details",
    );

    const state = await client.getState();
    const sessionFile = asString(
      state.sessionFile,
      "native get_state.data.sessionFile",
    );
    const rows = await loadJsonl(sessionFile);
    const compactionEntries = rows.filter((row) => row.type === "compaction");
    expect(
      compactionEntries.length > 0,
      "No native compaction entry was persisted",
    );
    const persistedDetails = nestedRecord(compactionEntries.at(-1)?.details);
    expect(
      persistedDetails.remoteCompaction === undefined,
      "Persisted native compaction entry contains OpenAI remote state",
    );
    ...
    expect(
      answer.includes(secret),
      `Native fallback lost continuity after compaction; got: ${answer}`,
    );
    console.log(`native fallback test passed (${primaryModel})`);
  } finally {
    await client.close();
  }
}
...
async function main(): Promise<void> {
  const artifactsRoot = await mkdtemp(
    join(tmpdir(), "pi-openai-compaction-live-"),
  );
  try {
    ...
    const reducedPlaintextWorkspaceDir = join(
      artifactsRoot,
      "reduced-plaintext-workspace",
    );
    ...
    const nativeFallbackDir = join(artifactsRoot, "native-fallback");
    await Promise.all([
      ...
      mkdir(nativeFallbackDir, { recursive: true }),
    ]);
    ...
    if (
      ...
      primaryModelProvider !== "openai-codex"
    ) {
      await runNativeFallbackTest(nativeFallbackDir, workspaceDir);
    ...
    if (primaryModelProvider === "openai") {
      await runReducedPlaintextReplayTest(
        reducedPlaintextDir,
        reducedPlaintextWorkspaceDir,
      );
    } else {
      console.log("== reduced-plaintext replay test ==");
