...
function tmuxSplit(direction: "right" | "down" = "right"): SplitTarget | null {
  try {
    ...
    const paneId = execSync(
      `tmux split-window ${flag} -c ${cwd} -P -F '#{pane_id}'`,
      { encoding: "utf8" },
    ).trim();
...
function tmuxTab(name?: string): SplitTarget | null {
  try {
    const cwd = shellQuote(process.cwd());
    const paneId = execSync(`tmux new-window -c ${cwd} -P -F '#{pane_id}'`, {
      encoding: "utf8",
    }).trim();
    if (name) {
...
function herderTab(name?: string): SplitTarget | null {
  try {
    ...
    const out = execSync(
      `herdr tab create --cwd ${cwd}${labelArg} --no-focus`,
      {
        encoding: "utf8",
      },
    ).trim();
...
function cmuxSplit(direction: "right" | "down" = "right"): SplitTarget | null {
  try {
    const out = execSync(`cmux new-split ${direction}`, {
      encoding: "utf8",
    }).trim();
...
function cmuxTab(name?: string): SplitTarget | null {
  try {
    ...
    if (name) {
      execSync(
        `cmux rename-tab --surface ${cmuxShellEscape(match[0])} ${cmuxShellEscape(name)}`,
      );
    }
...
function cmuxSendKeys(surfaceId: string, text: string): void {
  execSync(
    `cmux send --surface ${cmuxShellEscape(surfaceId)} ${cmuxShellEscape(text + "\n")}`,
  );
}
